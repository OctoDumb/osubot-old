...
export default function(score: APIRecentScore, beatmap: APIBeatmap, calc: ICalc, link: string) {
    ...
    let rebalancePP = score.mode === 0 ? new BanchoStdRebalance(beatmap, score).value : null;
...
${rebalancePP ? `Rebalance: ${rebalancePP.pp.toFixed(2)}${rebalancePP.ss == rebalancePP.pp ? '' : rebalancePP.fc == rebalancePP.pp ? ` ⯈ SS: ${rebalancePP.ss.toFixed(2)}` : ` ⯈ FC: ${rebalancePP.fc.toFixed(2)} ⯈ SS: ${rebalancePP.ss.toFixed(2)}`}` : ""}
