...
export default class RippleTop extends Command {
    constructor(module: Module) {
        super(["t", "top"], module, async (ctx, self, args) => {
            ...
            try {
                let user = await self.module.bot.api.ripple.getUser(dbUser.nickname);
                if(!dbUser.mode)
                    self.module.bot.database.servers.ripple.updateInfo(user, dbUser.mode || 0);
                if(args.apx) {
                    let top = await self.module.bot.api.ripple.getUserTop(dbUser.nickname, dbUser.mode || 0, 100);
                    let nearest = top[0];
                    let place = 1;
                    ...
                    let score = (await self.module.bot.api.ripple.getUserTop(dbUser.nickname, dbUser.mode || 0, args.place))[args.place - 1];
                    ...
                    let top = await self.module.bot.api.ripple.getUserTop(dbUser.nickname, dbUser.mode || 0, 3);
