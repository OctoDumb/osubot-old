    ...
    constructor(module: Module) {
        super(["t", "top"], module, async (ctx, self, args) => {
            ...
            try {
                ...
                if(!dbUser.mode)
                    self.module.bot.database.servers.gatari.updateInfo(user, dbUser.mode || 0);
                if(args.apx) {
                    let top = await self.module.bot.api.gatari.getUserTop(dbUser.nickname, dbUser.mode || 0, 100);
                    let nearest = top[0];
                    let place = 1;
                    ...
                    let top = await self.module.bot.api.gatari.getUserTop(dbUser.nickname, dbUser.mode || 0, 3);
