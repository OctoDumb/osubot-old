...
export default class VudekFind extends Command {
    constructor(module: Module) {
        super(["f", "find"], module, async (ctx, self, args) => {
            if(!args.nickname[0])
                return ctx.reply("Укажите ник!");
            try {
                let u = await self.module.bot.api.vudek.getUser(args.nickname.join(" "));
                let users = await self.module.bot.database.servers.vudek.findByUserId(u.id);
                if(!users[0])
