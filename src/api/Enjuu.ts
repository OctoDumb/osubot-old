...
class EnjuuTopScore implements APITopScore {
    api: IAPI;
    beatmapId: number;
    score: number;
    combo: number;
...
class EnjuuRecentScore implements APIRecentScore {
    api: IAPI;
    beatmapId: number;
    score: number;
    combo: number;
...
class EnjuuScore implements APIScore {
    api: IAPI;
    beatmapId: number;
    score: number;
    combo: number;
...
export default class EnjuuAPI implements IAPI {
    bot: Bot;
    ...
    async getUser(nickname: string, mode: number = 0): Promise<APIUser> {
        try {
            ...
            return new EnjuuUser(data[0], this);
        } catch(e) {
            throw e;
        }
    }
    ...
    async getUserTop(nickname: string, mode: number = 0, limit: number = 3): Promise<APITopScore[]> {
        try {
            ...
            return data.map(s => new EnjuuTopScore(s, mode, this));
        } catch(e) {
            throw e;
        }
    }
    ...
    async getUserRecent(nickname: string, mode: number = 0): Promise<APIRecentScore> {
        try {
            ...
            if(data[0])
                return new EnjuuRecentScore(data[0], mode, this);
            else
    ...
    async getScore(nickname: string, beatmapId: number, mode: number = 0, mods: number = null): Promise<APIScore> {
        ...
        try {
            ...
            return new EnjuuScore(data[0], mode, beatmapId, this);
        } catch(e) {
