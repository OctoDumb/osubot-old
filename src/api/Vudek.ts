...
class VudekUser implements APIUser {
    api: IAPI;
    id: number;
    nickname: string;
    playcount: number;
...
class VudekTopScore implements APITopScore {
    api: IAPI;
    beatmapId: number;
    score: number;
    combo: number;
...
class VudekRecentScore implements APIRecentScore {
    api: IAPI;
    beatmapId: number;
    score: number;
    combo: number;
...
class VudekScore implements APIScore {
    api: IAPI;
    beatmapId: number;
    score: number;
    combo: number;
...
export default class VudekAPI implements IAPI {
    bot: Bot;
    api: axios.AxiosInstance
    constructor(bot: Bot) {
        this.bot = bot;
        this.api = axios.default.create({
            baseURL: "https://osu.vudek.ga/api",
            timeout: 3000
        });
    }
    ...
    async getUser(nickname: string, mode: number = 0): Promise<APIUser> {
        try {
            ...
            return new VudekUser(data[0], this);
        } catch(e) {
            throw e;
        }
    }
    ...
    async getUserTop(nickname: string, mode: number = 0, limit: number = 3): Promise<APITopScore[]> {
        try {
            ...
            return data.map(s => new VudekTopScore(s, mode, this));
        } catch(e) {
            throw e;
        }
    }
    ...
    async getUserRecent(nickname: string, mode: number = 0): Promise<APIRecentScore> {
        try {
            ...
            if(data[0])
                return new VudekRecentScore(data[0], mode, this);
            else
    ...
    async getScore(nickname: string, beatmapId: number, mode: number = 0, mods: number = null): Promise<APIScore> {
        ...
        try {
            ...
            return new VudekScore(data[0], mode, beatmapId, this);
        } catch(e) {
